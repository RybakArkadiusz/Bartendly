...
public interface CocktailRepository extends JpaRepository<Cocktail, Long> {
    ...
    @Query(value = "SELECT c FROM Cocktail c WHERE " +
            ...
            "AND EXISTS (SELECT 1 FROM CocktailFlavourProfile cf WHERE cf.cocktail.id = c.id AND cf.flavourProfile IN :flavourNames GROUP BY cf.cocktail.id HAVING COUNT(cf.cocktail.id) = :numberOfFlavours) " +
            "AND EXISTS (SELECT 1 FROM CocktailNonAlcoholicIngredient cna WHERE cna.cocktail.id = c.id AND cna.nonAlcoholicIngredient.id IN :ingredientIds GROUP BY cna.cocktail.id HAVING COUNT(cna.cocktail.id) = :numberOfIngredients) ")
